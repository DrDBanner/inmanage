...
Defaults via CLI config:

- `INM_HEALTH_CHECK_INCLUDE=TAG1,TAG2` (optional include filter)
- `INM_HEALTH_CHECK_EXCLUDE=TAG1,TAG2` (optional exclude filter)
- If a tag appears in both, exclude wins.

Example:

```bash
inm env set cli INM_HEALTH_CHECK_EXCLUDE="FS,CRON"
```
